...
# Microservice 1 URL
ms1_url = 'https://p94b50rquh.execute-api.us-east-2.amazonaws.com/Beta/myresource/'
...
async def upload_video(username: str, file: UploadFile = File(...)):
    ...
    response = requests.post(f"{ms1_url}", files={"file": (new_file_name, file.file)})
...
async def fetch_video(username: str, video_id: str):
    ...
    if video and (video['uploader'] == username or video['publicity']):
        response = requests.get(f"{ms1_url}/{video_id}.mp4", stream=True)
        if response.status_code == 200:
...
async def delete_video(username: str, video_id: str):
    ...
    if video and video['uploader'] == username:
        response = requests.delete(f"{ms1_url}/{video_id}.mp4")
        if response.status_code == 200:
